...
int main()
{
 vector<vector<Mint>> nck(maxn, vector<Mint>(maxn));
 nck[0][0] = 1;
 ...
 vector<vector<Mint>> nck1(maxn, vector<Mint>(maxn));
 nck1[0][0] = 1;
 for(int i = 1; i < maxn; i++) {
  nck1[i][0] = 1;
  for(int j = 1; j < maxn; j++) nck1[i][j] += nck1[i][j-1] + nck[i][j];
 }
 int t;
 cin >> t;
 while(t--) {
  ...
  for(int i = 1; i <= n; i++) {
   int cnt = 0;
   if(mp.count(i)) cnt = mp[i];
   Mint ans = 0;
   for(int j = 1; j <= cnt; j++) {
    vector<Mint> ar;
    for(auto& k: mp) {
     if(k.first < i) {
      int tt = min(k.second, j-1);
      if(tt > 0) ar.push_back(nck1[k.second][tt] - 1);
     }
     else if(k.first > i) {
      int tt = min(k.second, j);
      if(tt > 0) ar.push_back(nck1[k.second][tt] - 1);
     }
    }
    debug(i, j, ar);
    Mint cur_ans = 1;
    for(auto& k: ar) k++, cur_ans *= k;
    cur_ans--;
    cur_ans *= nck[cnt][j];
    debug(cur_ans);
    ans += cur_ans;
   }
   ans += power((Mint) 2, cnt) - (Mint)1;
   cout << ans << ' ';
  }
  cout << '\n';
 }

...
def Runfor(Input, Output, ExpOutput, isDouble, DoubleTolerance):
	...
	if len(ExpOutput) != length:
		...
		for i in range (length):
			if isDouble == True:
				equal &= (abs (float(Output[i]) - float(ExpOutput[i])) <= DoubleTolerance)
			else:
...
def RunSample(command):
	...
	for I in range(totalTests):
		...
		totalCorrect += Runfor (Input, str (subprocess.run((command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, input=Input, encoding='ascii').stdout), str (Data['tests'][I]['output']), isDouble, DoubleTolerance)

	print()
	print (termcolor.colored("%s / %s Test Passed!" % (totalCorrect, totalTests), 'green' if totalCorrect == totalTests else 'red', attrs=['bold']))
	print (termcolor.colored("-----------", 'white', attrs=['bold']))
...
def AddSampleTestToJson(InputFileName):
	...
	TestData = subprocess.run (("cat %s" % InputFileName), shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').split("Output:\n")
...
def main():
	...
	if args.f == None:
		if args.add == None:
			sys.exit("Check Usage!!\nNeed arguments ['-h', '--help', '--f', '--add']")
		else:
			return

	fileinfo = args.f

	print ("fileinfo: " + termcolor.colored("%s" % fileinfo, 'white', 'on_magenta'))
	...
	if fileExtension == ".cpp":
		...
		if pathlib.Path('./%s' % executableName).exists() == False:
			...
			print ("No executable found for ", termcolor.colored("%s.cpp" % executableName, 'red', attrs=['bold']))
			exit(0)
		else:
			print ("Found Executable")
			Commandforexecution = "./%s" % executableName
	else:
		print ("Cannot run sample through " + termcolor.colored("%s" % fileinfo, 'red', attrs=['bold']))
		return 

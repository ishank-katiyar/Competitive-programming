...
template<class A> 
A rnd() {
 return uniform_int_distribution<A> (1, (int) 1e9) (rng);
}
...
.int main()
{
 ...
 while(t--) {
  ...
  for(int i = 0; i < n; i++) {
   ...
   string pref = "";
   for(char ch: s) {
    misspelled[pref+suf] = s;
    misspelled[pref+'?'+suf] = s;
    pref += ch;
    if(suf.empty() == false) suf.erase(suf.begin());
   }
  }
  while(q--) {
   string s;
   cin >> s;
   if(correct.count(s)) cout << s << '\n';
   else {
    reverse(s.begin(), s.end());
    string suf = s;
    reverse(s.begin(), s.end());
    ...
    for(char ch: s) {
     pref += ch;
     if(misspelled.count(pref+suf)) {
      cout << misspelled[pref+suf] << '\n';
      break;
     }
     pref.pop_back();
     if(misspelled.count(pref+'?'+suf)) {

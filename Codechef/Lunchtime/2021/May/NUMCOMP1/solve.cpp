...
public:
	...
	int rep (int N) {
		...
		return Parent[N] == N ? N : Parent[N] = rep(Parent[N]);
	}
...
int main() {
	...
	// int cnt_group = 0;
	// for (int i = 2; i < N; i++) {
	// 	// if (smallest_factor[i] == i) cnt_group += 1;
	// 	// auto pp = prime_factors (i);
	// 	// for (int j = 1; j < int(pp.size()); j++) {
	// 	// 	bool ok = d.Unite (pp[j], pp[j - 1]);
	// 	// 	cnt_group -= ok;
	// 	// }
	// 	// dp[i] = cnt_group;
	...
	while (t--) {
		...
		int x = upper_bound(prime.begin(), prime.end(), n / 2) - prime.begin();
		cout << total - x + 1 << '\n';
		continue;
	}
	return 0;

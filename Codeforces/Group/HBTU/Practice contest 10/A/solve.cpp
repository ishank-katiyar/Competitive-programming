...
int main()
{
 ...
 for(int i = 0; i < n; i++) {
  ...
  for(int j = 0; j < n; j++) {
   if(j == i) continue;
   if(a[j].first > a[i].first && a[j].second == a[i].second) f1 = true;
   if(a[j].first < a[i].first && a[j].second == a[i].second) f2 = true;
   if(a[j].first == a[i].first && a[j].second < a[i].second) f3 = true;
   if(a[j].first == a[i].first && a[j].second > a[i].second) f4 = true;
  }
  cnt += (f1 && f2 && f3 && f4);
 } 
 cout << cnt << '\n';

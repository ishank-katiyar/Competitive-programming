...
int main()
{
 ...
 for(int i = 1; i < maxn; i++) {
  for(auto& v: color[i]) {
   for(int u: adj[v]) {
    if(clr[u] != i) n_color[i].insert(clr[u]);
   }
  }
 }
 int mx = 0;
 for(int i = 1; i < maxn; i++) mx = max(mx, (int) n_color[i].size());
 ////assert(mx > 0);
 //if(mx == 0) {
  //cout << "1\n";
  //return 0;
 //}
 for(int i = 1; i < maxn; i++) {
  if(mx == (int) n_color[i].size() && (int) color[i].size() > 0) {
   cout << i << '\n';
   break;
  }
 }

...
int main()
{
 ...
 for(auto& i: a) {
  cin >> i;
  if(i % 2 == 1 && c != 1 && c != n) ind.push_back(c);
  c++;
 }
  ...
  if(n & 1) cout << "Yes\n";
   ...
   for(int i = 0; i < sz-1; i++) {
    //for(int j = i+2; j < sz-1; j++) {
     //if((ind[i+1]-ind[i]) == 1 && (ind[j+1]-ind[j]) == 1) {
      //if(ind[i] % 2 == 1 && (ind[j] - ind[i+1]) % 2 == 1 && (n - ind[j+1]) % 2 == 1) {
       //cout << "Yes\n";
       //return 0;
      //} 
     //} 
    //}
     ...
     if((ind[i+1]-ind[i]) == 1 && ind[i] % 2 == 1 && (n - ind[i+1]) % 2 == 1) cout << "Yes\n", ok = true;
   }
   if(ok == false) cout << "No\n";
  }
 }

...
int main()
{
 ...
 qq[n-1] += rr[n-1];
 for(int i = n-2; i >= 0; i--) rr[i] = max(rr[i], rr[i+1]), qq[i] += rr[i];
 pp[n-1] += qq[n-1];
 for(int i = n-2; i >= 0; i--) qq[i] = max(qq[i], qq[i+1]), pp[i] += qq[i];
 for(int i = n-2; i >= 0; i--) pp[i] = max(pp[i], pp[i+1]);
 debug(pp, qq, rr);
 cout << *max_element(pp.begin(), pp.end()) << '\n';
 return 0;
}

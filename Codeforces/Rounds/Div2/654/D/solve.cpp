...
int main()
{
 ...
 while(t--) {
  ...
  for(int i = 0; i < n; i++) {
   int ss = 0;
   for(int j = 0; j < n; j++) ss += a[j][i];
   cc[i] = ss;
   }
  sort(rr.begin() , rr.end());
  sort(cc.begin() , cc.end());
  assert(kk == (accumulate(rr.begin(), rr.end(), 0)) && kk == (accumulate(cc.begin(), cc.end(), 0)));
  ll ans = (rr[n-1] - rr[0]) * (rr[n-1] - rr[0]) + (cc[n-1] - cc[0]) * (cc[n-1] - cc[0]);
  cout << ans << '\n';
  for(auto& i: a) {
   for(int j: i) cout << j;
   cout << '\n';

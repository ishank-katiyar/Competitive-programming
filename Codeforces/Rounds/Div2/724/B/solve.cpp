...
int main() {
	...
	vector<string> ss;
	for (int i = 0; i < 26; i++) {
		ss.emplace_back (string(1, char('a' + i)));
	}
	for (int i = 0; i < 26; i++) {
		for (int j = 0; j < 26; j++) {
			string aa = string (1, 'a' + i) + string (1, 'a' + j);
			ss.push_back (aa);
		}
	}
	for (int i = 0; i < 26; i++) {
		for (int j = 0; j < 26; j++) {
			for (int k = 0; k < 26; k++) {
				string aa = string (1, 'a' + i) + string (1, 'a' + j) + string (1, 'a' + k);
				ss.push_back (aa);
			}
		}
	}
	...
	while (t--) {
		...
		string s;
		cin >> s;
		auto f = [] (const string &a, string b) -> bool {
			int n = a.size();
			int m = b.size();
			assert (n >= m);
			for (int i = 0; i <= n - m; i++) {

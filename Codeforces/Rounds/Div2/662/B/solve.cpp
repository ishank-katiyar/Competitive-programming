...
int main()
{
 ...
 while(t--) {
  ...
  map<int, int> mp;
  for(auto& i: a) mp[i]++;
  for(auto& i: mp) ms.insert(i.second);
  debug(mp, ms);
  int q;
  cin >> q;
  while(q--) {
   ...
   auto get = [] (multiset<int> &s) -> vector<int> {
    int l1 = *(--s.end());
    s.erase(--s.end());
    if(s.empty() == true) {
     s.insert(l1);
   ...
   vector<int> aa = get(ms);
   debug(aa);
   if(aa.empty() == true) cout << "NO\n";
   else {
    bool ok = false;
    if((int) aa.size() >= 1 && aa[0] >= 8) ok = true;
    if((int) aa.size() >= 2 && aa[0] >= 6 && aa[1] >= 2) ok = true;
    if((int) aa.size() >= 2 && aa[0] >= 4 && aa[1] >= 4) ok = true;
    if((int) aa.size() >= 3 && aa[0] >= 4 && aa[1] >= 2 && aa[2] >= 2) ok = true;
    cout << (ok ? "YES" : "NO") << '\n';
   }
  }
 }

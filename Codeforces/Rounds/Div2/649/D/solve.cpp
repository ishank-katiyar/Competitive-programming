...
void dfs(int x , int p) {
 ...
 for(int& i : adj[x]) {
 	if(i == p) continue;
 	if(used[i] == true) {
 		for(int j = 1; j <= n; j++) debug(j , adj[j]); 
 		debug(i , x);
 		cout << (int) component.size() << '\n';
 		for(int& j : component) cout << j << ' ';
 		exit(0);
 	}
 	dfs(i , x);
 }
...
int main()
{
	...
	for(int i = 1; i < maxn; i++) parent[i] = i , siz[i] = 1;
	cin >> n >> m >> k;	
	for(int i = 0; i < m; i++) {
		int x , y;
		cin >> x >> y;
	...
	if((int) odd.size() >= (k + 1) / 2) {
		while((int) odd.size() != (k + 1) / 2) odd.pop_back();
  for(int& i : odd) cout << i << ' ';
		return 0;
	}
	if((int) even.size() >= (k + 1) / 2) {
		while((int) even.size() != (k + 1) / 2) even.pop_back();

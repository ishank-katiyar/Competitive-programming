...
int main() {
	...
	int64_t r, g, b;
	cin >> r >> g >> b;
	vector<vector<vector<int64_t>>> dp (r + 1, vector<vector<int64_t>> (g + 1, vector<int64_t> (b + 1)));
	for (int i = 0; i <= r; i++) {
		for (int j = 0; j <= g; j++) {
			for (int k = 0; k <= b; k++) {
				int64_t x = -1;
				if (i > 0 && j > 0 && k > 0) x = dp[i - 1][j - 1][k - 1];
				if (i >= 2) {
					if (j > 0) x = max (x, dp[i - 2][j - 1][k]);
					if (k > 0) x = max (x, dp[i - 2][j][k - 1]);
				}
				if (j >= 2) {
					if (i > 0) x = max (x, dp[i - 1][j - 2][k]);
					if (k > 0) x = max (x, dp[i][j - 2][k - 1]);
				}
				if (k >= 2) {
					if (j > 0) x = max (x, dp[i][j - 1][k - 2]);
					if (i > 0) x = max (x, dp[i - 1][j][k - 2]);
				}
				dp[i][j][k] = x + 1;
				assert (dp[i][j][k] >= 0);

...
def RunforWithDiffEditor(TestCaseNumber, command, DiffEditor):
	...
	Output_command = "echo -en $(jq \".tests[%s].input\" < DATA_JSON.json | tr -d '\"') | %s" % (TestCaseNumber, command)
	# print (Output_command)
	with open (OutputFileName, 'w') as f:
		subprocess.run ((Output_command), shell=True, stdout=f)

	# correct output
	# echo $(jq ".tests[1].output" < DATA_JSON.json | tr -d '"') > correct_out.txt
	CorrectOutputFileName = "correct_out_test_case_%s" % (TestCaseNumber)
	CorrectOutputCommand = "echo -en $(jq \".tests[%s].output\" < DATA_JSON.json | tr -d '\"')" % (TestCaseNumber)
	# print (CorrectOutputCommand)
	with open (CorrectOutputFileName, 'w') as f:
		subprocess.run ((CorrectOutputCommand), shell=True, stdout=f)
	# delete last line (no need if echo run with flag -en)
	# sed -i '$d' correct_out.txt
	# diffeditor out.txt correct_out.txt
	DiffCommand = "%s %s %s" % (DiffEditor, OutputFileName, CorrectOutputFileName)
	# print (DiffCommand)
	returncode = subprocess.run ((DiffCommand), shell=True).returncode
	...
	#clean up
	CleanFiles = (DiffEditor is not "code --diff") and (DiffEditor is not "vimdiff")
	if CleanFiles:

...
int solver(string &s, char ch) {
 int n = (int) s.size();
 if(n == 1) return s[0] != ch;
 int x = n/2;
 string ss1 = s.substr(0,x);
 int cnt1 = count(ss1.begin(), ss1.end(), ch);
 cnt1 = x - cnt1;
 string ss2 = s.substr(x);
 int cnt2 = count(ss2.begin(), ss2.end(), ch);
 cnt2 = x - cnt2;
 debug(s,ch,ss1,ss2);
 return min(cnt1+solver(ss2,ch+1), solver(ss1,ch+1) + cnt2);
}

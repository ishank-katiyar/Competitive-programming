...
static const auto fast_io = [] {ios_base::sync_with_stdio(false); cin.tie(nullptr); cout.tie(nullptr); return nullptr;}();

mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
template<class A> 
A rnd() {return uniform_int_distribution<A> (1, (int) 1e9) (rng);}
template<class A> 
A rnd(A x, A y) {return uniform_int_distribution<A> (x, y) (rng);}
...
string to_string(tuple<T...> a) {
 ...
 return to_string(__vector_tuple);
}
...
int main()
{
 ...
 for(int i = 0; i < n; i++) {
  ll t,x,y;
  cin >> t >> x >> y;
  a[2*x+y].emplace_back(t);
 }
 sort(a[1].begin(), a[1].end());
 sort(a[2].begin(), a[2].end());
 sort(a[3].begin(), a[3].end());
 int sz1 = (int) a[1].size();
 for(int i = 1; i < sz1; i++) {
  a[1][i] += a[1][i-1];
 }
 int sz2 = (int) a[2].size();
 for(int i = 1; i < sz2; i++) {
  a[2][i] += a[2][i-1];
 }
 ll ans = 3e9;
 int sz3 = a[3].size();
 if(sz1 > k && sz2 > k) ans = a[1][k] + a[2][k];
 for(int i = 1; i < min(sz3, k+1); i++) {
  a[3][i] += a[3][i-1];
  if(sz1 > k-i && sz2 > k-i) ans = min(ans, a[3][i]+a[1][k-i]+a[2][k-i]);
 }
 if(ans == 3e9) cout << "-1";
 else cout << ans;
 cout << '\n';

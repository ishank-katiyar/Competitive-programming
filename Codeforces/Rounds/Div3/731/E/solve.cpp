...
int main() {
	...
	while (tt--) {
		...
		auto f = [] (int N, int K, vector<int> A, vector<int> T) -> vector<int> {
			...
			RangeQuery rmq (aa, [] (int x, int y) -> int { return min (x, y); });
			vector<int> ans (N, INT_MAX);
			for (int i = 0; i < N; i++) {
				int idx = lower_bound (A.begin(), A.end(), i + 1) - A.begin();
				if (idx >= K) continue;
				ans[i] = rmq.get (idx, K - 1) - i;
			}
			return ans;
		};
		vector<int> ans1 = f (n, k, a, t);
		debug (ans1)

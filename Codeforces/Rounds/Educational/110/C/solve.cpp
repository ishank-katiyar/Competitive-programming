...
int main() {
	...
	while (t--) {
		...
		// 	return a == b;
		// };
		// auto f = [&] (int idx, char ch) -> int {
		// 	for (int i = idx; i < n; i++) {
		// 		if (match (s[i], ch) == false) return i;
		// 		flip (ch);
		// 	}
		// 	return n;
		// };
		...
		// 	assert (idx <= n);
		// 	if (idx >= n) return n;
		// 	if (s[idx] == '0') return f (idx, s[idx]);
		// 	if (s[idx] == '1') return f (idx, s[idx]);
		// 	return max (f (idx, '0'), f (idx, '1'));
		// };
		// int j = 0;
		// int64_t ans = 0;
		// vector<int> dp (n);
		...
		if (s.back() == '0') dp.back().first = 1;
		if (s.back() == '1') dp.back().second = 1;
		if (s.back() == '?') dp.back().first = 1;
		if (s.back() == '?') dp.back().second = 1;
		ans += 1;
		for (int i = n - 2; i >= 0; i--) {
			if (s[i] == '0') dp[i].first = dp[i + 1].second + 1, ans += dp[i].first;
			if (s[i] == '1') dp[i].second = dp[i + 1].first + 1, ans += dp[i].second;
			if (s[i] == '?') dp[i].first = dp[i + 1].second + 1;
			if (s[i] == '?') dp[i].second = dp[i + 1].first + 1;
			if (s[i] == '?') ans += max (dp[i].first, dp[i].second);
		}
		debug (dp)
		cout << ans << '\n';
	}

...
int main() {
	...
	while (t--) {
		...
		auto dfs = y_combinator ([&] (auto self, int i, int j, char ch) -> void {
			if (i - 1 >= 0 && a[i - 1][j] == '.') {
				a[i - 1][j] = ch;
				self (i - 1, j, flip (ch));
			}
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				if (i - 1 >= 0) ok &= (a[i][j] != a[i - 1][j]);
				if (i + 1 < n) ok &= (a[i][j] != a[i + 1][j]);
				if (j + 1 < m) ok &= (a[i][j] != a[i][j + 1]);
				if (j - 1 >= 0) ok &= (a[i][j] != a[i][j - 1]);
			}
		}
		if (ok == false) {
			cout << "NO" << '\n';

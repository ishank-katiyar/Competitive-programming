...
int main() {
	...
	for (int ttt = 1; ttt <= TT; ttt++) {
		...
		int64_t pre_sum = 0;
		int64_t prev = 1;
		map<int64_t, vector<pair<int64_t, int64_t>>> mp1;
		for (auto& i: mp) {
			if (i.first == 1) {
				pre_sum += i.second;
				prev = 1;
			}	else {
				mp1[pre_sum].push_back(make_pair (prev, i.first - 1));
				pre_sum += i.second;
				prev = i.first;
			}
		}
		...
		for (auto it = --mp1.end(); ; it--) {
			...
			for (auto& j: vv) {
				if (c > 0) {
					debug (c, it->first, vv)
					int64_t mn = min (c, j.second - j.first + 1);
					// cut_points.push_back (make_pair (j.first, j.first + mn - 1));
					ans += mn * it->first;
					c -= mn;
				}
			}
		...
		// int sz = int(cut_points.size());
		// for (int i = 1; i < sz; i++) {
		// 	assert (cut_points[i].first > cut_points[i - 1].second);
		// }
		// for (int i = 0; i < n; i++) {

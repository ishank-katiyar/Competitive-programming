...
class StarsInTheSky {
public:
 bool check(vector<int> inside, vector<int> outside, vector<int> &x, vector<int> &y) {
  debug(inside, outside);
  int min_x = INT_MAX, max_x = -1, min_y = INT_MAX, max_y = -1;
  for(int idx: inside) {
   min_x = min(min_x, x[idx]);
   max_x = max(max_x, x[idx]);
   min_y = min(min_y, y[idx]);
   max_y = max(max_y, y[idx]);
  }
  for(int idx: outside) {
   if((x[idx] >= min_x && x[idx] <= max_x) && (y[idx] >= min_y && y[idx] <= max_y)) {
    debug(idx, min_x, max_x, min_y, max_y);
    return false;
   }
  }
  return  true;
 ...
 int countPictures(int N, vector <int> X, vector <int> Y) {
   ...
   for(int mask = 1; mask < (1LL << N); mask++) {
    ...
    for(int i = 0; i < N; i++) {
     if((mask >> i) & 1) inside.push_back(i);
     else outside.push_back(i);
    }
    assert(inside.empty() == false);
    if(check(inside, outside, X, Y)) {
      //debug(inside, outside);
      ans++;
    }
   }
...
int main()
{
 ...
 cout << s.countPictures((int) x.size(), x, y) << '\n';
 return 0;
}
#endif

...
int main() {
	...
	while (t--) {
		...
		for (int i = 1; i <= n; i++) {
			int b = 1;
			for (auto& f: prime_factors (i)) {
				if ([] (int I, int F) -> int {
					int cnt = 0;
					while (I % F == 0) cnt += 1, I /= F;
					return cnt;
				}(i, f) % 2 == 1) b *= f;
			}
			mp[b].push_back (i);
		...
		vector<int> ans (n + 1);
		for (auto& i: mp) {
			assert (is_sorted (i.second.begin(), i.second.end()));
			vector<pair<int, int>> aa;
			for (auto& j: i.second) {
				aa.push_back (make_pair (a[j - 1].second, j));
			}
			sort (aa.begin(), aa.end());
			int sz = i.second.size();
			assert (int(aa.size()) == sz);
			for (int j = 0; j < sz; j++) {
				ans[i.second[j]] = aa[j].first;
			}
		}
		debug (ans);
		bool ok = true;

...
void dfs(int n, int cur, vector<vector<int>> &adj, vector<bool> &used, vector<vector<char>> &ans) {
 if(used[n] == true) return;
 used[n] = true;
 ans[cur][n] = 'Y';
 for(auto& i: adj[n]) {
  dfs(i, cur, adj, used, ans);
 }
}
...
void solve() {
 ...
 for(int i = 1; i <= n; i++) {
  if(O[i-1] == 'Y') {
   if(i > 1) {
    if(I[i-2] == 'Y') adj[i].push_back(i-1);
   }
   if(i < n) {
    if(I[i] == 'Y') adj[i].push_back(i+1);
   }
  }
 }
 debug(adj);
 vector<vector<char>> ans(n+1, vector<char> (n+1, 'N'));
 vector<bool> used(n+1);
 for(int i = 1; i <= n; i++) {
  used.assign(n+1, false);
  dfs(i, i, adj, used, ans);
 }
 for(int i = 1; i <= n; i++) {
